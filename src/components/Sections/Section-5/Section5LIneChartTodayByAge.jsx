...
const Section5LineChartTodayByAge = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTime(
          jsonData.timeWiseTodayRegReport.map((item) => {
        ...
        setTotalDoses(
          jsonData.timeWiseTodayRegReport.map((item) => item.total)
        );
        setTwelveTo14(
          jsonData.timeWiseTodayRegReport.map((item) => item.age12)
        );
        setFifteenTo17(
          jsonData.timeWiseTodayRegReport.map((item) => item.age15)
        );
        setEighteenTo44(
          jsonData.timeWiseTodayRegReport.map((item) => item.age18)
        );
        setFortyfiveTo60(
          jsonData.timeWiseTodayRegReport.map((item) => item.age45)
        );
        setAbove60(jsonData.timeWiseTodayRegReport.map((item) => item.age60));
      } catch (error) {
...
export default Section5LineChartTodayByAge;

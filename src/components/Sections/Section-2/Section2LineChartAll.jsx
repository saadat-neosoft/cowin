  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setTime(jsonData.weeklyReport.map((item) => item.label));
        setTotalDoses(jsonData.weeklyReport.map((item) => item.total));
        setDoseOne(jsonData.weeklyReport.map((item) => item.dose1));
        setDoseTwo(jsonData.weeklyReport.map((item) => item.dose2));
        setPrecautionDose(jsonData.weeklyReport.map((item) => item.dose_pd));
      } catch (error) {
...
export default Section2LineChartAll;
